...
val emptyMap = Map.empty[String, ExpectedReport] + (initialisePrefix ~> emptyReport)
...
val files = ISZ[C](
  ...
  C(tcDir / "TempControlSoftwareSystem" / "TempControl_s_tcproc_tempControl.scala", defaultOpts, 
    emptyMap + ("handle_fanAck" ~> emptyReport) + ("handle_setPoint" ~> emptyReport) + ("handle_tempChanged" ~> emptyReport)),

  C(tcDir / "TempSensor" / "TempSensor_s_tcproc_tempSensor.scala", defaultOpts, emptyMap + (timeTriggeredPrefix ~> emptyReport)),
)
...
var result: Z = 0
for(f <- files;
    entryPoint <- f.expectedReports.keys()) {
