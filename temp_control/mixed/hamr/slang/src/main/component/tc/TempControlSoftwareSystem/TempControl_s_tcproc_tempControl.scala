  ...
  def handle_fanAck(api: TempControl_s_Operational_Api, value : CoolingFan.FanAck.Type): Unit = {
    Contract(
      ...
      Ensures(
        ...
        fanError ->: (In(currentFanState) == currentFanState &&
           (api.fanCmd.nonEmpty &&
        ...
        (api.fanAck.get == CoolingFan.FanAck.Ok) ->: !fanError &
          (api.fanAck.get == CoolingFan.FanAck.Error) ->: fanError
        // END COMPUTE ENSURES fanAck
      )
    )
    value match {
      case CoolingFan.FanAck.Ok =>
        // Log actuation succeeded
        api.logInfo("Fan actuation worked.")
        fanError = F
      case CoolingFan.FanAck.Error =>
