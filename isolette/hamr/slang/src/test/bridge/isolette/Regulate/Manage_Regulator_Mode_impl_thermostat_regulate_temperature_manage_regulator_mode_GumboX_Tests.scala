...
class Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_GumboX_Tests extends Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_GumboX_TestHarness_ScalaTest {
  ...
  def getTestVector(): Option[Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_DSC_TestVector] = {
    try {
      ...
      return Some(Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_DSC_TestVector(api_current_tempWstatus,api_interface_failure,api_internal_failure))
    } catch {
  ...
  def getTestVectorwL(): Option[Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_DSC_TestVectorwL] = {
    try {
      ...
      return Some(Manage_Regulator_Mode_impl_thermostat_regulate_temperature_manage_regulator_mode_DSC_TestVectorwL(In_lastRegulatorMode,api_current_tempWstatus,api_interface_failure,api_internal_failure))
    } catch {
      case e: AssertionError =>
       // SlangCheck was unable to satisfy a datatype's filter
       return None()
        ...
        while (j < GumboXUtil.numTestVectorGenRetries && retry) {
          getTestVectorwL() match {
            case Some(o) =>
              ...
              if (verbose) {
                println(st"""${if (j > 0) s"Retry $j: " else ""}Testing with
                            |    lastRegulatorMode = $o.In_lastRegulatorMode
                            |    current_tempWstatus = $o.api_current_tempWstatus
                            |    interface_failure = $o.api_interface_failure
                            |    internal_failure = $o.api_internal_failure""".render)
              }

              testComputeCBwL(o.In_lastRegulatorMode, o.api_current_tempWstatus, o.api_interface_failure, o.api_internal_failure) match {
                case GumboXResult.Pre_Condition_Unsat =>
                case GumboXResult.Post_Condition_Fail =>
                  fail ("Post condition did not hold")
                  retry = F
